...
- **项目层 = 跟随项目**：具体项目接入件（协议挂载/run/项目文档/项目链/profile）——不进框架模板。
- **目标**：其他游戏设计师用同一套模式，积累**完全不同的**技能原子/电子书/真实项目。
- **原则**："别人拿去的是模式，不是你的答案"（视频 ⑧、结尾）。判定细则与现状清单见 [`FRAMEWORK-LAYERING.md`](./FRAMEWORK-LAYERING.md)（归属判定三问/双层合一处理/空包组成预告）；落地待办：H 组（分层审计 ✅ → 空框架包 → 新用户向导 → 个人层独立性）。
...
README（对外门面：这是什么/怎么用/快速开始）
  └─ POSITIONING（定位·纲领·内部权威，本文档）
        ...
        ├─ FRAMEWORK-LAYERING 模式/内容/项目三层（可迁移性判定与现状清单——分发/接入时读）
