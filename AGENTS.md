...
## 0.0 启动铁律（2026-08-24 定案：防"新会话忘记架构精神"）

任何会话启动**按序执行**，顺序不得颠倒：
1. 读本文件（运行时协议）
2. **读 [`FRAMEWORK-MIND.md`](./FRAMEWORK-MIND.md) 完整段**（架构精神/三实体矩阵/红线——不得跳、不得只读指针）
3. 按任务数据入口恢复（读书→book.json+读书日志+书地图；设计→run 检查点+叙事链）
4. 本轨规范（reading→READING-PROTOCOL §1+§3.3；涉及原子判定→GOVERNANCE 四维+边界四问）

**启动自检**（说不出口=未完成启动）：①三实体（骨架/判据/原子）区别一句话？②本次判定依据是执行文本还是名字？
...
**`reading` 必读**（每节阅读轮）：①`books/<slug>/book.json` 摘要（元信息/状态/游标/未读清单）②最近 N 轮 `读书日志.md` 摘要（上次读到哪、怎么看的）③当前节正文（只带这一节）④相关现有判据/原子对照（查重）。执行手册见 [`READING-PROTOCOL.md`](./READING-PROTOCOL.md)。⑤**规范状态必读**（§0.0 步骤 2/4）：`FRAMEWORK-MIND.md` 完整段 + READING-PROTOCOL §1（三通道分流/预检门槛）+ §3.3（结算唯一真相源）；涉及原子判定加 GOVERNANCE 四维+边界四问。
...
## 6. 会话管理（2026-08-23 用户定案）

**对话是执行通道，状态在文件。** 一个波次 = 一个会话；波次完整结束（入库+回写完成）或上下文明显见顶时，**果断结束会话、开新对话**——新会话经恢复入口（读书日志顶部/AGENTS.md §2）从文件状态无缝续接，不硬扛长会话（长会话上下文腐烂、注意力衰减，早期细节只占空间不再被用）。判断时机：①一轮工作完整收尾 ②"早期细节记不清了"（记忆衰减信号）③计划跨波次继续时。可视化会话地图等工具只是"视觉甜品"——不替代"状态在文件"的架构精神，不作升级方向。
